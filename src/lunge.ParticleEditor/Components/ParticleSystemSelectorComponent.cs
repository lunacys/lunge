...
namespace lunge.ParticleEditor.Components;

public class ParticleSystemSelectorComponent : Component, IUpdatable
{
	...
	{
		_particleConfigs = Directory.GetFiles("Content/Particles", "*.pex");
	...
	void CreateUi()
	{
        ...
        rightTable.Left().Top().PadTop(4);
        rightTable.SetFillParent(true);
        rightTable.Defaults().SetPadTop(4).SetPadRight(4).SetPadLeft(4).SetAlign(Align.Left);
        rightTable.SetBackground(new PrimitiveDrawable(new Color(40, 40, 40, 220)));

        // wrap up the table in a ScrollPane
        var scrollPaneRight = uiCanvas.Stage.AddElement(new ScrollPane(rightTable, skin));
        scrollPaneRight.SetPosition(Screen.Width - 340, 0);//( 400, 400, 400, 400);
        ...
        //uiCanvas.Stage.AddElement(rightTable);

        MakeSection(rightTable, skin, "Sprite Info");
        rightTable.Row();
        rightTable.Add(new Label("Sprite Texture")).Width(120);
        //table.Row();


        var sprite = _particleEmitterConfig.Sprite;

        var spriteTextureStr = sprite?.Texture2D?.Name ?? "None";

        rightTable.Add(new Label($"{spriteTextureStr}"));

        rightTable.Add(new Image(sprite, Scaling.Stretch)).SetMaxWidth(64).SetMaxHeight(64);
        rightTable.Row();
        rightTable.Add(new TextButton("Remove Texture", skin))
            .Width(100)
            .Height(20)
            .GetElement<Button>()
            .OnClicked += b =>
        {
            _particleEmitterConfig.Sprite = Graphics.Instance.PixelTexture.Clone();
        };

        // Stolen from RuntimeInspector.cs
        var table = new Table();
        table.Top().Left().PadTop(60);
        ...
        /*var tabStyle = new TabStyle
        {
            Background = new PrimitiveDrawable(Color.Cyan),
        };

        var tabPane = table.Add(new TabPane(new TabWindowStyle
        {
			Background = new PrimitiveDrawable(Color.White),
			TabButtonStyle = new TabButtonStyle
            {
				Active = new PrimitiveDrawable(Color.Blue),
				Hover = new PrimitiveDrawable(Color.DarkBlue),
				Inactive = new PrimitiveDrawable(Color.Gray),
				LabelStyle = new LabelStyle(Color.Black),
				Locked = new PrimitiveDrawable(Color.DarkGray)
            }
        })).GetElement<TabPane>();
        tabPane.AddTab(new Tab("Test 1", tabStyle));
        tabPane.AddTab(new Tab("Test 2", tabStyle));
        tabPane.AddTab(new Tab("Test 3", tabStyle));
        tabPane.AddTab(new Tab("Test 4", tabStyle));*/
        ...
        var windowTabsTable = uiCanvas.Stage.AddElement(new Table());
        windowTabsTable.Top().Left();
        windowTabsTable.Defaults().SetPadTop(0).SetAlign(Align.Left);
        windowTabsTable.SetBackground(new PrimitiveDrawable(new Color(255, 40, 40, 255)));
        windowTabsTable.SetHeight(40f);

        windowTabsTable.Add(new Label($"{_particleConfigs[_currentParticleSystem]}"));
        windowTabsTable.Row().SetPadTop(4);

        windowTabsTable.Add(new TextButton("Prev", skin))
            .GetElement<Button>().OnClicked += b => Prev();
        windowTabsTable.Row().SetPadTop(4);
        windowTabsTable.Add(new TextButton("Next", skin))
            .GetElement<Button>().OnClicked += b => Next();
        ...
        var vg = windowTabsTable.Add(new HorizontalGroup(4)).Pad(4, 4, 4, 4).GetElement<HorizontalGroup>();
        vg.AddElement(new TextButton("New", skin));
        vg.AddElement(new TextButton("Open", skin));
        vg.AddElement(new TextButton("Save", skin));
        vg.AddElement(new TextButton("Hello", skin));
        vg.AddElement(new TextButton("Exit", skin));

		/*var tabPane = windowTabsTable.Add(new TabPane(new TabWindowStyle
        {
            Background = new PrimitiveDrawable(Color.White),
            TabButtonStyle = new TabButtonStyle
            {
        ...
        })).Pad(0, 4, 0, 4).GetElement<TabPane>();
        tabPane.AddTab(new Tab("Test 1", tabStyle));
        tabPane.AddTab(new Tab("Test 2", tabStyle));
        tabPane.AddTab(new Tab("Test 3", tabStyle));
        tabPane.AddTab(new Tab("Test 4", tabStyle));*/
