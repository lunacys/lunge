...
namespace lunge.Library.Scheduling
{
    ...
    public class TaskHandler<T, TResult> : IDisposable
    {
        ...
        /// </summary>
        public event EventHandler<TaskCompletedEventArgs<T, TResult>> OnCompleted;
        ...
        private LogHelper _logger => LoggerFactory.GetLogger("TaskHandler");
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    OnCompleted?.Invoke(typeof(TaskHandler<T, TResult>),
                        new TaskCompletedEventArgs<T, TResult>(input, result));
                }
