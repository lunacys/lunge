...
namespace lunge.Library.Particles
{
    ...
    public abstract class ParticleModifierExecutionStrategy
    {
        ...
        {
            if (string.Equals(nameof(ParallelModifierExecutionStrategy), value, StringComparison.OrdinalIgnoreCase))
                return Parallel;

            if (string.Equals(nameof(SerialModifierExecutionStrategy), value, StringComparison.OrdinalIgnoreCase))
                return Serial;
