...
public class InMemoryScript
{
    ...
    private InMemoryScript(InMemoryScriptTypes type, string scriptPath,
        string scriptValue) // init only with static methods
    {
        Type = type;
    ...
    public IEnumerable<KeyValuePair<string, string>> GetScript()
    {
        ...
        else if (Type == InMemoryScriptTypes.TextFileEncodedInBase64)
        {
            ...
            return new List<KeyValuePair<string, string>>
                { new KeyValuePair<string, string>(ScriptPath, Encoding.UTF8.GetString(data)) };
        }
            ...
            using MemoryStream
                stream = new MemoryStream(
                    Convert.FromBase64String(
                        ScriptValue)); // decode the Base64 string to a stream, in memory  // see https://stackoverflow.com/questions/25919387/converting-file-into-base64string-and-back-again
            using ZipArchive
                zip = new ZipArchive(stream); // open the zip file  // see https://stackoverflow.com/a/22605118/5288052
            foreach (ZipArchiveEntry entry in
                     zip.Entries) // loop zip file entries, saving only files (entry.Name not empty)
            {
                if (!(string.IsNullOrEmpty(entry
                        .Name))) // see https://docs.microsoft.com/en-us/dotnet/api/system.io.compression.ziparchiveentry.name
                {
                    using StreamReader sr = new StreamReader(entry.Open());
                    string zipContent = sr.ReadToEnd();
                    retList.Add(new KeyValuePair<string, string>($"{ScriptPath}{entry.FullName}",
                        zipContent)); // save path and content from the zip file, prepending 'ScriptPath' to the path
                }
            }
